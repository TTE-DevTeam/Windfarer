...
public class ForbiddenBlockValidator implements DetectionPredicate<MovecraftLocation> {
    ...
    public @NotNull Result validate(@NotNull MovecraftLocation movecraftLocation, @NotNull TypeSafeCraftType type, @NotNull MovecraftWorld world, @Nullable Entity pilot) {
        return type.get(PropertyKeys.FORBIDDEN_BLOCKS).contains(NamespacedIDUtil.getBlockID(world.getData(movecraftLocation))) ? Result.failWithMessage(I18nSupport.getInternationalisedString("Detection - Forbidden block found")) : Result.succeed();
    }
}
